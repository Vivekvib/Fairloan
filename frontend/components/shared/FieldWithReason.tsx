...
interface FieldWithReasonProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> {
  label: string;
  reason: string;
  error?: string;
  helpText?: string;
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        ...
        >
          {showReason ? <X size={14} aria-hidden="true" /> : <HelpCircle size={14} aria-hidden="true" />}
        </button>
      </div>

      {showReason && (
        <div role="note" aria-live="polite"
          className="text-xs text-slate-600 bg-blue-50 border border-blue-100 rounded-md px-3 py-2">
          {reason}
        </div>
      )}
      ...
      <div className="relative flex items-center">
        <input
          ...
          className={clsx(
            "w-full rounded-md border px-4 py-3 text-sm text-slate-900",
      ...
      {error && (
        <p id={`${inputId}-error`} role="alert" aria-live="assertive"
          className="text-xs text-red-600 font-medium">{error}</p>
      )}
    </div>
  );
}
